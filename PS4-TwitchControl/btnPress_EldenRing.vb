    ...
    Private Sub execCMD_EldenRing(user As String, role As String, cmd As String)
        ...
        Select Case cmd
            ...
            Case "l1", "lb"
                ...
                Return
            Case "l2", "lt"
                If duration = 0 Then duration = 80
            ...
            Case "cr2", "crt"
                If duration = 0 Then duration = 50
                Controller(BTN_R2, 0, 0, 0, 0, 0, 1, 70, user, cmd & "(!)")
                Controller(0, 0, 0, 0, 0, 0, 0, duration, user, cmd & "(-)")
                Return
        ...
        If ((cmd(0) = "w") Or (cmd(0) = "l") Or (cmd(0) = "j") Or (cmd(0) = "a")) Or
            ...
            If roll Then
                Controller(BTN_O, axis(2), axis(3), axis(0), axis(1), 0, 0, 5, user, cmd & "(!)")
                Controller(0, axis(2), axis(3), axis(0), axis(1), 0, 0, duration, user, cmd & "(-)")
            ElseIf jump Then
                Controller(BTN_X, axis(2), axis(3), axis(0), axis(1), 0, 0, 5, user, cmd & "(!)")
                Controller(0, axis(2), axis(3), axis(0), axis(1), 0, 0, duration, user, cmd & "(-)")
            Else
                Controller(0, axis(2), axis(3), axis(0), axis(1), 0, 0, duration, user, cmd)
            End If
